...
`serve.sh`를 계속 띄워 두려면 Docker 사용을 권장합니다. 재부팅·Docker 재시작 후에도 자동 기동됩니다.

```bash
docker compose up -d --build   # 백그라운드 실행 (최초 1회 빌드)
docker compose logs -f         # 로그 확인
docker compose down            # 종료
```

- 포트는 호스트의 `127.0.0.1:9191`로만 공개돼 **로컬 전용**을 유지합니다(컨테이너 내부는 `HOST=0.0.0.0` 바인딩).
- 프로젝트 디렉터리를 컨테이너에 마운트하므로, 호스트 훅이 쓰는 `activity.jsonl`·`approvals/`를 그대로 읽고 웹 승인 결정도 호스트에 반영됩니다.
